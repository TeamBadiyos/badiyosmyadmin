...
**Radius parity with dispatch (confirmed):** the capacity check uses the exact same radius the dispatch broadcast would use at that moment — the booking's `current_search_radius_km` if a broadcast has already expanded it, otherwise the per-city initial radius, falling back the same way `broadcast_booking_to_experts` does. As the background job expands a booking's radius over time, re-evaluated capacity follows the expanded radius, so a customer is never told "zero capacity" while an expert inside the widened radius exists.

**How "almost available" is detected:** for every busy expert who is online, eligible for the booking's category and inside that same effective radius, the expected free-at time is their job's start time plus its booked duration. If that lands within the configurable window (default 10 minutes), the zone counts as "almost available".
