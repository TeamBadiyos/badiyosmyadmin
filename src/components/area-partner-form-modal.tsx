...
export function AreaPartnerFormModal({
  partnerId,
  onClose,
}: {
  ...
  const fetchPartner = useServerFn(getAreaPartner);
  ...
  const { data: existing } = useQuery({
    ...
    queryFn: () => fetchPartner({ data: { id: partnerId! } }),
    enabled: !!partnerId,
  });
  ...
  const [fee, setFee] = useState<"pending" | "paid">("pending");
  ...
  useEffect(() => {
    if (existing && !seededRef.current) {
      ...
      setFee(existing.setupFeeStatus);
      setRate(String(existing.commissionRate ?? 0));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await sign({ data: { bucket: "area-partner-photos", path: photoPath } });
      ...
      >
        <header className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <h2 className="text-[18px] font-bold text-foreground">
            {isEdit ? "Edit Area Partner" : "Add Area Partner"}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          <section className="flex items-center gap-4">
            ...
            <FileButton
              ...
              onFile={async (file) => {
                try {
                  const path = await uploadTo("area-partner-photos", file, "photos");
                  setPhotoPath(path);
                } catch (e) {
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <SelectField label="Setup fee status" value={fee} onChange={(v) => setFee(v as "pending" | "paid")}>
            ...
            <SelectField label="Status" value={status} onChange={(v) => setStatus(v as "active" | "inactive")}>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <KycUpload label="Aadhaar" doc={aadhaar} onUploaded={setAadhaar} onError={setFormError} />
              <KycUpload label="PAN" doc={pan} onUploaded={setPan} onError={setFormError} />
              <KycUpload label="Address proof" doc={addressProof} onUploaded={setAddressProof} onError={setFormError} />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {ifscError && <p className="mt-1 text-[12px] text-destructive">{ifscError}</p>}
              </div>
        ...
        <footer className="px-6 py-4 border-t border-border flex items-center justify-end gap-3 shrink-0">
          ...
          >
            <Check size={16} />
            {saveMutation.isPending ? "Saving…" : isEdit ? "Save changes" : "Create partner"}
          </button>
        </footer>
      </div>
    </div>
...
function KycUpload({
  ...
  return (
    <div className="border border-border rounded-[14px] p-3 flex flex-col gap-2">
      ...
      {doc.name && <p className="text-[12px] text-muted-foreground truncate">{doc.name}</p>}
      <FileButton
