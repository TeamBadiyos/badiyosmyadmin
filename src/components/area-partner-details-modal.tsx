...
export function AreaPartnerDetailsModal({
  partnerId,
  role,
  onClose,
  onEdit,
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => fetch({ data: { id: partnerId } }),
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await sign({ data: { bucket: "area-partner-photos", path: data.photoUrl! } });
  ...
  const mutation = useMutation({
    mutationFn: (payload: { decision: PartnerKycStatus; reason?: string }) =>
      decide({ data: { partnerId, decision: payload.decision, reason: payload.reason } }),
    onSuccess: () => {
  ...
  async function openDoc(path: string) {
    try {
      const res = await sign({ data: { bucket: "area-partner-kyc-docs", path } });
    ...
    >
      ...
      >
        <header className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Area Partner</p>
          ...
          <div className="flex items-center gap-2">
            ...
            {onDelete && !isDeleted && (
              <button
                onClick={onDelete}
                className="h-10 px-3 rounded-[12px] border border-destructive text-destructive font-semibold text-[13px] inline-flex items-center gap-2 hover:bg-red-50"
              >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          {isError && (
            <p className="text-[14px] text-destructive">
              {(error as Error)?.message ?? "Failed to load area partner."}
            </p>
          )}
          {data && (
            <>
              {isDeleted && (
                <p className="text-[13px] text-red-700 bg-red-50 border border-red-100 rounded-[12px] p-3">
                  <span className="font-bold uppercase text-[11px] tracking-wide mr-2">Deleted</span>
                  {data.deleteReason ?? "No reason recorded"}
                </p>
              )}
              ...
              <section className="flex items-start gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge className={data.setupFeeStatus === "paid" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}>
                    Setup fee {data.setupFeeStatus}
                  </Badge>
                  <Badge className="bg-slate-100 text-slate-700">Commission {data.commissionRate}%</Badge>
                </div>
              </section>
              ...
              <section className="bg-background border border-border rounded-[18px] p-4">
                ...
                {canManage && !isDeleted && data.kycStatus === "pending" && !rejectOpen && (
                ...
                {canManage && !isDeleted && rejectOpen && (
                ...
                {canReset && !isDeleted && data.kycStatus !== "pending" && (
                  <button
                    disabled={mutation.isPending}
                ...
                {actionError && <p className="mt-3 text-[12px] text-destructive">{actionError}</p>}
              </section>
            </>
          )}
        </div>
