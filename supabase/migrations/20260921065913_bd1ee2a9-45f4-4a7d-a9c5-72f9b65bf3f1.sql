...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      expert_id := _actor.id;
      SELECT e.name INTO expert_name FROM public.experts e WHERE e.id = _actor.id;
...
REVOKE EXECUTE ON FUNCTION public.staff_reward_period_preview(text, date) FROM PUBLIC, anon;
