...
CREATE OR REPLACE FUNCTION public.staff_update_support_ticket(
  _ticket_id uuid, _status text, _note text DEFAULT NULL::text,
  _resolution text DEFAULT NULL::text, _resolution_outcome text DEFAULT NULL::text)
RETURNS void
...
BEGIN
  ...
  IF _resolution_outcome IS NOT NULL
     AND _resolution_outcome NOT IN ('expert_fault','customer_fault','platform_issue','no_fault','other') THEN
  ...
  UPDATE public.support_tickets
     SET status = _status,
         ...
         resolution_outcome = CASE WHEN _status = 'resolved' THEN COALESCE(_resolution_outcome, resolution_outcome) ELSE resolution_outcome END,
  ...
  VALUES(_uid, 'update_support_ticket', 'support_tickets', _ticket_id, _before,
         jsonb_build_object('status', _status, 'internal_note', _note,
                            'resolution_summary', _resolution,
                            'resolution_outcome', _resolution_outcome));
END $function$;

REVOKE EXECUTE ON FUNCTION public.staff_update_support_ticket(uuid, text, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.staff_update_support_ticket(uuid, text, text, text, text) TO authenticated, service_role;
DROP FUNCTION IF EXISTS public.staff_update_support_ticket(uuid, text, text, text);
